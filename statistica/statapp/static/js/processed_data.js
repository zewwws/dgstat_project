...
// temporary debug line, remove after
        ...
        filteredData.forEach(row => {
            ...
            allKeys.forEach(key => {
                ...
                if (typeof cellContent === 'string' && (cellContent.includes('\n') || cellContent.includes('\r'))) {
                    const lines = cellContent.replace(/\r\n/g, '\n').replace(/\r/g, '\n').split('\n');
                    td.innerHTML = lines.map(line => {
                        const match = line.match(/^(.*?)(\s*\([^)]+\)\s*)$/);
                        if (match) {
                            return `<div style="display:flex; justify-content:space-between; gap:1rem;">
                                        <span style="text-align:right; flex:1;">${match[1].trim()}</span>
                                        <span style="text-align:left; white-space:nowrap;">${match[2].trim()}</span>
                                    </div>`;
                        }
                        return `<div style="text-align:right;">${line}</div>`;
                    }).join('');
                } else {
    ...
    function processData(filtered) {
        ...
        return filtered.map((row, index) => {
            console.log(`--- Row ${index + 1}/${filtered.length} | MED: ${row['MED'] || 'NO_MED'} | AVVIATO: ${row['AVVIATO'] || 'NO_DATE'}`);
    
            const [campo42, campo43] = campo42_43(row);
            const campi_12_27 = processCampi_12_27(row);
            const [campo30, campo31] = campo30_31(row);  // ← call the function
            console.log('campo30:', campo30, '| campo31:', campo31); // ← debug check
    ...
    function processCampi_12_27(row) {
        ...
        entriesNumbered.forEach(entry => {
            const hasIstante = /istante/i.test(entry);
            const hasAderente = /aderente/i.test(entry);
            const hasInvitata = /invitat[ao]/i.test(entry);
            const hasAvv = /avv\./i.test(entry);
    ...
    function campo30_31(row) {
        ...
        const key = Object.keys(materiaMapping).find(
            k => k.toLowerCase() === materia.toLowerCase()
        );

        const campo30 = key ? materiaMapping[key] : 'Unknown';
        const campo31 = campo30 === '15' ? (row['NOTE'] || '').split(/[\r\n]/)[0].trim() : '';
        return [campo30, campo31];
    }

    // campo32: "VALORE", format values as required
    function campo32(row) {
        const value = (row['VALORE'] || '').trim().toLowerCase();

        const ranges = {
            'variabile':              '0,00',
            'indeterminato':          '0,00',
            'indeterminabile (basso)':  '0,00',
            'indeterminabile (medio)':  '0,00',
            'indeterminabile (alto)':   '0,00',
            'fino 1.000':             '500,00',
            '1.001 - 5.000':          '3000,50',
            ...
            'oltre 5.000.000':        '5000001,00'
        };

        return ranges[value] || '0';
    }

    // 4 functions + 3 auxiliary functions for spese istante and adrente before and after reductions/increases
    ...
    function generateCSV(processedData, trimester) {
        ...
        const seenCampo06 = new Set();

        const rows = processedData
            .filter(row => {
                // Check date validity
                const validDates = dateRegex.test(row['campo07']) && dateRegex.test(row['campo08']);
                if (!validDates) {
                    console.warn('⚠️ Excluded (invalid dates):', row);
                    return false;
                }

                // Check campo06 uniqueness
                if (seenCampo06.has(row['campo06'])) {
                    console.warn('⚠️ Excluded (duplicate campo06):', row['campo06'], row);
                    return false;
                }
                seenCampo06.add(row['campo06']);
        ...
        // Final file name
        const fileName = `V7D_${xxxx}_${yyyy}_${tt}${aa}.csv`;
    ...
    document.getElementById('process_riepilogo').addEventListener('click', function() {
        ...
        allCategoryMappings = generateAllCategoryMappings(allData, selectedTrimester, categoryMappingFunctions, trimester);
        ...
        // Build snapshots payload
        const trimesterKey = getTrimesterKey(trimester.startDate);
        console.log("Sample mapping entry:", Object.entries(allCategoryMappings)[1]);
        const snapshots = Object.entries(allCategoryMappings)
            .filter(([materia, counts]) => counts._pendenti_finali !== undefined)  // skip Constants & Unique Mediator
            .map(([materia, counts]) => ({
                materia,
                pendenti_iniziali: counts._pendenti_iniziali,
                pendenti_finali: counts._pendenti_finali,
            }));

        fetch('/save_trimester_snapshot/', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ trimester: trimesterKey, snapshots })
        })
        .then(r => r.json())
        .then(data => {
            if (data.warnings && data.warnings.length > 0) {
                const warningHtml = data.warnings.map(w =>
                    `<div class="warning">⚠️ ${w.materia}: ${w.message}</div>`
                ).join('');
                document.getElementById("warningContainer").innerHTML = warningHtml;
            } else {
                document.getElementById("warningContainer").innerHTML = 
                    `<div class="success">✅ ${trimesterKey} salvato, nessuna discrepanza.</div>`;
            }
        })
        .catch(err => console.error('❌ Snapshot save error:', err));
    });

    function getTrimesterKey(startDate) {
    ...
    document.getElementById('download_csv').addEventListener('click', function() {
        if (!allCategoryMappings) {
            alert('Please run the summary process first.');
            return;
        }
    ...
    function countDataByTrimesterForCategory(data, selectedTrimester, category, trimester) {
        ...
        allData.forEach(row => {
            ...
            // ── AVVIATO IN TRIMESTER ───────────────────────────────────────
            if (avviatoVal !== null && avviatoVal >= trimStartVal && avviatoVal <= trimEndVal) {
            ...
            if (conclusoVal !== null && conclusoVal >= trimStartVal && conclusoVal <= trimEndVal) {
                ...
                console.log(`>> CONCLUSO HIT | ${category} | MED: ${row.MED} | AVVIATO: ${row.AVVIATO} | CONCLUSO: ${row.CONCLUSO} | ESITO: ${esito}`);
                if (esito === "ACCORDO") {
                    counts.esitoAccordo++;
            ...
            if (primoIncVal !== null && primoIncVal >= trimStartVal && primoIncVal <= trimEndVal) {
                counts.primoInc++;
        ...
        if (counts.giudiceSi > counts.avviatoInTrimester) {
            console.error(
                `⚠️ ANOMALIA ${category} | giudiceSi (${counts.giudiceSi}) > avviatoInTrimester (${counts.avviatoInTrimester})`
            );
        }
        return counts;
    }
    ...
    function generateAllCategoryMappings(data, selectedTrimester, categoryMappingFunctions, trimester) {
        ...
        Object.keys(categoryMappingFunctions).forEach(category => {
            ...
            if (mapFn && counts) {
                ...
                // inject raw values for DB snapshot, invisible to buildFlussiTable
                finalCategoryMappings[category]._pendenti_iniziali = counts.pendenti_iniziali;
                finalCategoryMappings[category]._pendenti_finali = counts.pendenti_finali;
            } else {
    ...
    return {
        ...
        campo170: counts.primoInc, 
        campo171: counts.primoIncOltre,
        campo172: counts.primoIncNegativo,      
    };
    }
    ...
    return {
        ...
        campo174: counts.primoInc,             
        campo175: counts.primoIncOltre,    
        campo176: counts.primoIncNegativo,      
    };
    }
    ...
    return {
        ...
        campo178: counts.primoInc, 
        campo179: counts.primoIncOltre,
        campo180: counts.primoIncNegativo,
    };
    }
    ...
    return {
        ...
        campo182: counts.primoInc, 
        campo183: counts.primoIncOltre,
        campo184: counts.primoIncNegativo, 
    };
    }
    ...
    return {
        ...
        campo186: counts.primoInc,
        campo187: counts.primoIncOltre,
        campo188: counts.primoIncNegativo,
    };
    }
    ...
    return {
        ...
        campo190: counts.primoInc,
        campo191: counts.primoIncOltre,
        campo192: counts.primoIncNegativo,        
    };
    }
    ...
    return {
        ...
        campo194: counts.primoInc,
        campo195: counts.primoIncOltre,
        campo196: counts.primoIncNegativo,        
    };
    }
    ...
    return {
        ...
        campo198: counts.primoInc,
        campo199: counts.primoIncOltre,
        campo200: counts.primoIncNegativo,        
    };
    }
    ...
    return {
        ...
        campo202: counts.primoInc,
        campo203: counts.primoIncOltre,
        campo204: counts.primoIncNegativo,        
    };
    }
    ...
    return {
        ...
        campo206: counts.primoInc,
        campo207: counts.primoIncOltre,
        campo208: counts.primoIncNegativo   ,        
    };
    }
    ...
    return {
        ...
        campo210: counts.primoInc,
        campo211: counts.primoIncOltre,
        campo212: counts.primoIncNegativo,        
    };
    }
    ...
    return {
        ...
        campo214: counts.primoInc,
        campo215: counts.primoIncOltre,
        campo216: counts.primoIncNegativo,        
    };
    }
    ...
    return {
        ...
        campo218: counts.primoInc,
        campo219: counts.primoIncOltre,
        campo220: counts.primoIncNegativo,        
    };
    }
    ...
    return {
        ...
        campo229: counts.primoInc,
        campo230: counts.primoIncOltre,
        campo231: counts.primoIncNegativo,        
    };
    }
    ...
    return {
        ...
        campo240: counts.primoInc,
        campo241: counts.primoIncOltre,
        campo242: counts.primoIncNegativo,        
    };
    }
    ...
    return {
        ...
        campo251: counts.primoInc,
        campo252: counts.primoIncOltre,
        campo253: counts.primoIncNegativo,        
    };
    }
    ...
    return {
        ...
        campo262: counts.primoInc,
        campo263: counts.primoIncOltre,
        campo264: counts.primoIncNegativo,   
    };
    }
    ...
    return {
        ...
        campo273: counts.primoInc,
        campo274: counts.primoIncOltre,
        campo275: counts.primoIncNegativo,        
    };
    }
    ...
    return {
        ...
        campo284: counts.primoInc,
        campo285: counts.primoIncOltre,
        campo286: counts.primoIncNegativo,        
    };
    }
    ...
    return {
        ...
        campo314: counts.primoInc,
        campo315: counts.primoIncOltre,
        campo316: counts.primoIncNegativo,        
    };
    }
    ...
    return {
        ...
        campo325: counts.primoInc,
        campo326: counts.primoIncOltre,
        campo327: counts.primoIncNegativo,    
    };
    }
    ...
    return {
        ...
        campo303: counts.primoInc,
        campo304: counts.primoIncOltre,
        campo305: counts.primoIncNegativo,        
    };
    }
    ...
    function buildFlussiTable(allCategoryMappings) {
        ...
        tableDefinition.forEach(def => {
            const mapping = allCategoryMappings[def.name] || {};
            const [pi, isc, giud, ma, acc, mnacc, def_, pf, primo, oltre, neg] = def.campos.map(c => mapping[c] || 0);
            totals.pendentiIniziali += pi;
            totals.iscritti += isc;
            totals.giudiceSi += giud;
            totals.mancataAdesione += ma;
            totals.accordo += acc;
            totals.mancatoAccordo += mnacc;
            totals.definitiTotale += def_;
            totals.pendentiFinali += pf;
            totals.primoInc += primo;
            totals.primoIncOltre += oltre;
            totals.primoIncNegativo += neg;
        });

        html += `
            <tr style="font-weight:bold; background-color:#f0f0f0;">
                <td>TOTALE</td>
                <td>${totals.pendentiIniziali}</td>
                <td>${totals.iscritti}</td>
                <td>${totals.giudiceSi}</td>
                <td>${totals.mancataAdesione}</td>
                <td>${totals.accordo}</td>
                <td>${totals.mancatoAccordo}</td>
                <td>${totals.definitiTotale}</td>
                <td>${totals.pendentiFinali}</td>
                <td>${totals.primoInc}</td>
                <td>${totals.primoIncOltre}</td>
                <td>${totals.primoIncNegativo}</td>
            </tr>
        `;
